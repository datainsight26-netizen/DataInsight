    (() => {
        ...
        function renderCharts(data) {
            ...
            Object.assign(margemOpts, {
                series: [{ name: 'Margem de contribuição', data: m.map(x => Number((x.margemPct || 0).toFixed(2))) }],
                colors: [c.success],
                ...
                tooltip: { shared: false, intersect: false, y: { formatter: (v) => pct(v) } }
            });
        ...
        function setTab(tab) {
            ...
            if (currentData) {
                if (tab === 'geral' || tab === 'anual') setTimeout(() => renderCharts(currentData), 0);
                if (tab === 'anual') setTimeout(() => renderScenarioComparison(), 0);
            }
        }
